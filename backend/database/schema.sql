...
-- 37. OAuth anti-CSRF state records (formerly stored in Firestore 'oauth_states').
...
-- 38. One-time OAuth exchange codes (formerly stored in Firestore 'oauth_exchange_codes').
...
-- 39. Durable PDF export render tokens (formerly memory-only with optional Firestore
--     persistence). MySQL is authoritative; tokens are single-use with TTL expiry.
...
-- 40. Password-reset tokens (formerly Firestore 'password_reset_tokens').
...
-- 42. Email-verification tokens (formerly Firestore 'email_verifications').
...
-- 44. Daily AI usage quota (formerly Firestore 'ai_usage'). MySQL authoritative;
