...
function enterpriseContext({ tenantId = crypto.randomUUID(), workspaceId = crypto.randomUUID(), principalId = crypto.randomUUID(), subjectId = 'outbox-subject' } = {}) {
  return freezeContext({
    ...
    dataPlane: { id: 'mysql-primary', type: 'MYSQL', region: 'default', routingVersion: 1, storageProfile: 'shared', cacheProfile: 'shared', queueProfile: 'mysql-transactional-outbox', aiProfile: 'platform-default', securityProfile: 'standard' },
  });
}

function setup() { return { pool: new InMemoryEnterpriseOutboxPool() }; }

function serviceFor(context) {
  return { resolveContext: async () => ({ context }) };
}

function runWorker({ pool, context, handlers, now, workerId = 'w1', signingSecret = SECRET, maxJobs = 5 }) {
  return outbox.runOutboxWorkerOnce({ pool, tenantService: serviceFor(context), signingSecret, handlers, workerId, now, maxJobs, backoffBaseMs: 1_000, backoffJitter: false });
}
...
test('enqueue is idempotent: duplicate submissions collapse onto one durable job', async () => {
  ...
  const first = await outbox.enqueueOutboxJob({ pool, envelope });
  const second = await outbox.enqueueOutboxJob({ pool, envelope });
  assert.equal(first.status, 'ENQUEUED');
  assert.equal(second.status, 'ALREADY_QUEUED');
  assert.equal(second.jobId, first.jobId);
  await assert.rejects(
    () => outbox.enqueueOutboxJob({ pool, envelope: { ...envelope, resource: { type: 'RESUME', id: 'different-resource' } } }),
    error => error.code === 'ENTERPRISE_OUTBOX_IDEMPOTENCY_CONFLICT' && error.status === 409
  );
  await assert.rejects(
    () => outbox.enqueueOutboxJob({ pool, envelope: { ...envelope, policyVersion: envelope.policyVersion + 1 } }),
    error => error.code === 'ENTERPRISE_OUTBOX_IDEMPOTENCY_CONFLICT' && error.status === 409
  );

  // Completing then re-delivering the same envelope never re-executes.
  const outcomes = await runWorker({ pool, context, handlers: { EXPORT_PDF: async () => ({ ok: 1 }) }, now: Date.now() });
  assert.equal(outcomes[0].status, 'COMPLETED');
  const afterCompletion = await outbox.enqueueOutboxJob({ pool, envelope });
  assert.equal(afterCompletion.status, 'DUPLICATE_IGNORED', 'duplicate delivery after completion must be ignored');
  const secondPass = await runWorker({ pool, context, handlers: { EXPORT_PDF: async () => { throw new Error('must not run'); } }, now: Date.now() });
...
test('expired jobs are terminally rejected by the sweeper and never executed', async () => {
  ...
  await outbox.enqueueOutboxJob({ pool, envelope, ttlMs: 60_000 });
  const tomorrow = Date.now() + 25 * 60 * 60_000;
  const swept = await outbox.sweepExpiredJobs({ pool, now: tomorrow });
  assert.equal(swept.length, 1);
  assert.equal(swept[0].reason, 'EXPIRED_TENANT_JOB');
  const outcomes = await runWorker({ pool, context, handlers: { EXPORT_PDF: async () => ({ ok: true }) }, now: tomorrow });
...
test('membership revocation between enqueue and execution rejects the job', async () => {
  ...
  await outbox.enqueueOutboxJob({ pool, envelope });
  ...
  const outcomes = await outbox.runOutboxWorkerOnce({ pool, tenantService: revokedService, signingSecret: SECRET, handlers: { NOTIFY: async () => ({ ok: true }) }, workerId: 'w1', now: Date.now(), maxJobs: 3, backoffBaseMs: 1, backoffJitter: false });
  assert.equal(outcomes[0].status, 'REJECTED');
  assert.equal(outcomes[0].reason, 'TENANT_MEMBERSHIP_NOT_FOUND');
  const status = await outbox.getOutboxStatus({ pool, signingSecret: SECRET });
...
test('stale principal binding (identity swap) rejects the job at execution time', async () => {
  ...
  const outcomes = await outbox.runOutboxWorkerOnce({ pool, tenantService: swappedService, signingSecret: SECRET, handlers: { NOTIFY: async () => ({ ok: true }) }, workerId: 'w1', now: Date.now(), maxJobs: 3, backoffBaseMs: 1, backoffJitter: false });
  assert.equal(outcomes[0].status, 'REJECTED');
...
test('DLQ: persistent failures dead-letter after max attempts and replay re-queues them', async () => {
  ...
  await outbox.enqueueOutboxJob({ pool, envelope, maxAttempts: 3 });
  const failing = { EXPORT_PDF: async () => { throw new Error('renderer permanently offline'); } };

  let clock = Date.now();
  const first = await runWorker({ pool, context, handlers: failing, now: clock });
  assert.equal(first[0].status, 'RETRY_SCHEDULED');
  clock = first[0].nextRunAt;
  const second = await runWorker({ pool, context, handlers: failing, now: clock });
  assert.equal(second[0].status, 'RETRY_SCHEDULED');
  clock = second[0].nextRunAt;
  const third = await runWorker({ pool, context, handlers: failing, now: clock });
  ...
  const adminContext = freezeContext({
    ...
    tenant: { id: context.tenantId, lifecycleState: 'ACTIVE', isolationTier: 'STANDARD', dataPlane: { id: 'mysql-primary', type: 'MYSQL', routingVersion: 1 } },
    membership: { status: 'ACTIVE', roles: ['TENANT_ADMIN'] }, roles: ['TENANT_ADMIN'], permissions: ['tenant.settings.write'],
  });
  const replayed = await outbox.replayDeadLetterJob({ pool, jobId, context: adminContext });
  assert.equal(replayed.status, 'REPLAYED');

  const fixed = { EXPORT_PDF: async () => ({ exported: true }) };
  const fourth = await runWorker({ pool, context, handlers: fixed, now: Date.now() + 1_000 });
  ...
  assert.ok(pool.auditEvents.some(event => event.tenantId === context.tenantId && event.action === 'JOB_REPLAYED'), 'JOB_REPLAYED audit event must exist');
});
...
test('replay is tenant-scoped: tenant B cannot replay tenant A dead letters', async () => {
  ...
  await outbox.enqueueOutboxJob({ pool, envelope, maxAttempts: 1 });
  const failing = { EXPORT_PDF: async () => { throw new Error('boom'); } };
  const [outcome] = await runWorker({ pool, context: contextA, handlers: failing, now: Date.now() });
  ...
  const contextB = freezeContext({
    ...
    tenant: { id: crypto.randomUUID(), lifecycleState: 'ACTIVE', isolationTier: 'STANDARD', dataPlane: { id: 'mysql-primary', type: 'MYSQL', routingVersion: 1 } },
    membership: { status: 'ACTIVE', roles: ['TENANT_ADMIN'] }, roles: ['TENANT_ADMIN'], permissions: ['tenant.settings.write'],
  });
  const jobId = outbox.outboxDocumentId(contextA.tenantId, 'cross-1');
  const denied = await outbox.replayDeadLetterJob({ pool, jobId, context: contextB });
  assert.equal(denied.status, 'NOT_FOUND', 'cross-tenant replay must be invisible (not-found, not forbidden)');

  const allowed = await outbox.replayDeadLetterJob({ pool, jobId, context: freezeContext({ tenantId: contextA.tenantId, workspaceId: contextA.workspaceId, principalId: crypto.randomUUID(), tenant: { id: contextA.tenantId, lifecycleState: 'ACTIVE', isolationTier: 'STANDARD', dataPlane: { id: 'mysql-primary', type: 'MYSQL', routingVersion: 1 } }, membership: { status: 'ACTIVE', roles: ['TENANT_ADMIN'] }, roles: ['TENANT_ADMIN'], permissions: [] }) });
...
test('tampered envelopes (payload or signature swap) are rejected without execution', async () => {
  ...
  pool.jobs.get(jobId).envelope = JSON.stringify({ ...envelope, tenantId: crypto.randomUUID() });
  let outcomes = await runWorker({ pool, context, handlers: { NOTIFY: async () => ({ ok: true }) }, now: Date.now() });
  assert.equal(outcomes[0].status, 'REJECTED');
  ...
  await outbox.enqueueOutboxJob({ pool, envelope: envelope2 });
  outcomes = await runWorker({ pool, context, handlers: { NOTIFY: async () => ({ ok: true }) }, now: Date.now(), signingSecret: ALT_SECRET });
...
test('job listing is tenant-scoped and ordered', async () => {
  ...
  for (const key of ['list-a-1', 'list-a-2']) {
    await outbox.enqueueOutboxJob({ pool, envelope: createTenantJobEnvelope({ context: contextA, jobType: 'NOTIFY', resource: { type: 'RESUME', id: 'r' }, idempotencyKey: key, signingSecret: SECRET }) });
  }
  await outbox.enqueueOutboxJob({ pool, envelope: createTenantJobEnvelope({ context: contextB, jobType: 'NOTIFY', resource: { type: 'RESUME', id: 'r' }, idempotencyKey: 'list-b-1', signingSecret: SECRET }) });

  const listA = await outbox.listOutboxJobs({ pool, context: contextA, status: null });
  assert.equal(listA.length, 2);
  assert.ok(listA.every(job => job.tenantId === contextA.tenantId));
  const listB = await outbox.listOutboxJobs({ pool, context: contextB, status: 'QUEUED' });
...
test('missing handlers retry and dead-letter instead of dropping jobs silently', async () => {
  ...
  const [outcome] = await runWorker({ pool, context, handlers: {}, now: Date.now() });
  assert.equal(outcome.status, 'DEAD_LETTER', 'unhandled job types must be visible in the DLQ, not dropped');
  const [job] = await outbox.listOutboxJobs({ pool, context, status: 'DEAD_LETTER' });
  assert.match(job.lastError, /No handler registered/);
});

test('outbox status fails closed when MariaDB is unavailable', async () => {
  const unavailable = { query: async () => { throw Object.assign(new Error('connection refused'), { code: 'ECONNREFUSED' }); } };
  await assert.rejects(() => outbox.getOutboxStatus({ pool: unavailable, signingSecret: SECRET }), error => error.code === 'ECONNREFUSED');
});
