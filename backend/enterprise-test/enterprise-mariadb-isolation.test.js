...
process.env.TENANT_JOB_SIGNING_SECRET = 'enterprise-isolation-test-signing-secret-32-bytes';
...
/**
 * Adversarial API isolation suite using explicit test doubles for the immutable
 * MariaDB registry, repository, quota, credential, grant, and outbox contracts.
 */
...
function installMariaDbContractService() {
  ...
  const outboxPool = new InMemoryEnterpriseOutboxPool();
  setPoolForTests(outboxPool);
  const service = new TenantService({
    registry, repository, serviceAccountStore, supportGrantStore,
    quotaGuard: new TenantQuotaGuard({ store: new InMemoryAtomicCounterStore() }),
    encryptionProvider, dataProviderName: 'mysql',
  });
  app.set('tenantService', service);
  return { service, registry, repository, serviceAccountStore, supportGrantStore, outboxPool };
}
...
test('IDOR/BOLA: cross-tenant resource access is structurally impossible through the API', async () => {
  const { service } = installMariaDbContractService();
...
test('service accounts: tenant A key cannot act in tenant B and revocation is immediate', async () => {
  const { service, serviceAccountStore } = installMariaDbContractService();
  const owner = await request(app).get('/api/enterprise/context').set('Authorization', bearer('alice'));
  const tenantId = owner.body.context.tenantId;
  ...
  assert.ok(!JSON.stringify(serviceAccountStore).includes(apiKey), 'plaintext key must never be persisted');
...
test('AI quota enforcement is durable and tenant-isolated without any Redis in the process', async () => {
  ...
  const first = await request(app).post('/api/enterprise/ai/generate-content').set(headers).send({
    operation: 'generate-summary',
    payload: { jobTitle: 'Engineer', sourceFacts: 'Maintained verified deployment runbooks' },
  });
  assert.ok([403, 502].includes(first.status), `first call consumed quota then failed closed (${first.status})`);

  const second = await request(app).post('/api/enterprise/ai/generate-content').set(headers).send({
    operation: 'generate-summary',
    payload: { jobTitle: 'Engineer', sourceFacts: 'Maintained verified deployment runbooks' },
  });
...
test('durable queue endpoints enforce tenant scoping for listing and replay', async () => {
  const { outboxPool, service } = installMariaDbContractService();
  const owner = await request(app).get('/api/enterprise/context').set('Authorization', bearer('alice'));
  const tenantId = owner.body.context.tenantId;
  ...
  assert.equal(status.body.queue.engine, 'mariadb-transactional-outbox');
  assert.equal(outboxPool.jobs.size, 1, 'durable queue contract captured the tenant job');
});
