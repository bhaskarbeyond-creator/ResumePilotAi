...
test('MariaDB chaos contract: data-store outage fails closed — the quota guard errors instead of allowing unbounded use', async () => {
  const unavailableStore = { increment: async () => { throw Object.assign(new Error('simulated MariaDB outage'), { code: 'ECONNREFUSED' }); } };
  const guard = new TenantQuotaGuard({ store: unavailableStore });
  ...
  await assert.rejects(
    ...
    error => error.code === 'ECONNREFUSED' && /simulated MariaDB outage/i.test(error.message),
    'quota consumption during a store outage must fail closed, never silently allow'
  );
});

test('MariaDB chaos contract: worker crash mid-execution is recovered after lease expiry by another worker', async () => {
  ...
  const claimed = await outbox.claimNextOutboxJob({ pool, workerId: 'crashed-worker', now: Date.now(), leaseMs: 1_000 });
  assert.ok(claimed, 'job must be claimed by the crashing worker');

  // While the lease is held, another worker cannot steal the job.
  const stolen = await outbox.claimNextOutboxJob({ pool, workerId: 'rescue-worker', now: Date.now() + 100, leaseMs: 1_000 });
  ...
  const reclaimed = await outbox.claimNextOutboxJob({ pool, workerId: 'rescue-worker', now: Date.now() + 5_000, leaseMs: 1_000 });
...
test('MariaDB chaos contract: transient handler failures retry with backoff and eventually succeed', async () => {
  const pool = new InMemoryEnterpriseOutboxPool();
  ...
  await outbox.enqueueOutboxJob({ pool, envelope, maxAttempts: 5 });
  ...
  const first = await outbox.runOutboxWorkerOnce({ pool, tenantService, signingSecret: SIGNING_SECRET, handlers, workerId: 'w1', now: clock, maxJobs: 3, backoffBaseMs: 1_000, backoffJitter: false });
  assert.equal(first[0].status, 'RETRY_SCHEDULED');
  clock = first[0].nextRunAt;
  const second = await outbox.runOutboxWorkerOnce({ pool, tenantService, signingSecret: SIGNING_SECRET, handlers, workerId: 'w1', now: clock, maxJobs: 3, backoffBaseMs: 1_000, backoffJitter: false });
  ...
  const third = await outbox.runOutboxWorkerOnce({ pool, tenantService, signingSecret: SIGNING_SECRET, handlers, workerId: 'w1', now: clock, maxJobs: 3, backoffBaseMs: 1_000, backoffJitter: false });
  assert.equal(third[0].status, 'COMPLETED');
  assert.equal(attempts, 3);
});

test('MariaDB chaos contract: suspended or revoked tenants are terminally rejected at execution time', async () => {
  const pool = new InMemoryEnterpriseOutboxPool();
  ...
  const outcomes = await outbox.runOutboxWorkerOnce({
    pool, tenantService: suspendedService, signingSecret: SIGNING_SECRET,
  ...
  const status = await outbox.getOutboxStatus({ pool, signingSecret: SIGNING_SECRET });
  assert.equal(status.counts.REJECTED, 1, 'deterministic rejection must not retry');
});

test('MariaDB chaos contract: Artifact storage token rejects expired credentials and mismatched purpose', async () => {
