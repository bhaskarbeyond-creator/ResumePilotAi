...
const { assertSameInfrastructureRoute, resolveTenantInfrastructure } = require('./tenantRouting');
...
function verifyTenantArtifactToken({ context, token, purpose, signingSecret, now = Date.now() }) {
  ...
  assertSameInfrastructureRoute(context, claims);
  if (claims.storageProfile !== resolveTenantInfrastructure(context).storageProfile) {
    throw Object.assign(new Error('Tenant artifact storage profile mismatched'), { code: 'TENANT_ROUTE_MISMATCH', status: 409 });
  }
