...
const { buildCoverLetterPrompt, coverLetterFields, validateCoverLetterOutput } = require('./services/coverLetterAi');
        ...
        const providerRes = await fetch(`${baseUrl}/v2/checkout/orders`, {
            ...
            body: JSON.stringify({
                ...
                purchase_units: [{
                    ...
                    description: `${req.body.planId} IME365 subscription`,
...
app.post('/api/subscription/preferences', (_req, res) => {
    ...
    return res.status(410).json({
        ...
        error: 'IME365 plans are fixed-term one-time purchases and do not renew automatically.',
        requestId: res.locals.requestId,
    });
});
...
app.post('/api/generate-ai-cover-letter', async (req, res) => {
    ...
    try {
        const fields = coverLetterFields(req.body || {});
        if (!fields.title && !fields.jobDescription) {
            return res.status(400).json({ success: false, error: { code: 'COVER_LETTER_INPUT_REQUIRED', message: 'Add the job title or job description so the letter can be tailored.', requestId: res.locals.requestId } });
        }
        ...
        try {
            ...
            // Usage is billed for the provider call whether or not the draft passes validation.
            ...
            const checked = validateCoverLetterOutput(providerResult.raw, fields);
            if (checked.ok) {
                ...
                return res.json({ success: true, coverLetter: checked.coverLetter, provider: providerResult.provider });
            }
            failureCode = 'AI_OUTPUT_REJECTED';
            console.warn('[Cover letter output rejected]', { reason: checked.reason, requestId: res.locals.requestId });
        } catch (providerError) {
            if (providerError?.status && providerError.status < 500 && providerError.code) throw providerError;
            console.warn('[Cover letter provider unavailable]', { code: providerError.code || 'PROVIDER_ERROR', requestId: res.locals.requestId });
        }
        // Explicit unavailable state — no template letter is ever substituted.
        return res.status(503).json({ success: false, aiUnavailable: true, error: { code: failureCode, message: 'AI cover letter generation is unavailable right now. Please try again.', requestId: res.locals.requestId } });
    } catch (error) {
        if (error?.status >= 400 && error.status < 500 && error.code) {
            return res.status(error.status).json({ success: false, error: { code: error.code, message: error.message, requestId: res.locals.requestId } });
        }
...
app.get('/api/linkedin-scraper', async (req, res) => {
    ...
    try {
        ...
        const context = await browser.newContext({
            userAgent: 'IME365JobsBot/1.0 (+https://ime365.com)',
...
const DEFAULT_BUILTIN_PAGES = {
    ...
    'terms-of-service': {
        ...
        pagecontent: `<div style="font-family: 'Inter', system-ui, sans-serif; color: #1e293b; line-height: 1.7; max-width: 860px; margin: 0 auto; padding: 40px 20px;"><div style="margin-bottom: 32px; border-bottom: 1px solid #e2e8f0; padding-bottom: 24px;"><span style="font-size: 13px; font-weight: 800; color: #1a73e8; text-transform: uppercase; letter-spacing: 0.06em;">Terms of Agreement</span><h1 style="font-size: 36px; font-weight: 800; color: #0f172a; margin: 8px 0 12px 0;">Terms of Service</h1><p style="color: #64748b; font-size: 14px; margin: 0;">Effective Date: September 2026</p></div><section style="margin-bottom: 32px;"><h2 style="font-size: 20px; font-weight: 700; color: #0f172a; margin-bottom: 12px;">1. Acceptance of Terms</h2><p>By accessing IME365, you agree to these Terms of Service, all applicable laws, and regulations.</p></section><section style="margin-bottom: 32px;"><h2 style="font-size: 20px; font-weight: 700; color: #0f172a; margin-bottom: 12px;">2. User Accounts & Subscriptions</h2><p>Free accounts receive access to foundational ATS resume templates and standard AI tailoring. Pro Subscriptions unlock full access to all 51 certified templates.</p></section></div>`,
        published: true,
        status: 'published'
    },
    'cookie-policy': {
    ...
    'about-us': {
        ...
        pagecontent: `<div style="font-family: 'Inter', system-ui, sans-serif; color: #1e293b; line-height: 1.7; max-width: 860px; margin: 0 auto; padding: 40px 20px;"><div style="margin-bottom: 32px; border-bottom: 1px solid #e2e8f0; padding-bottom: 24px;"><span style="font-size: 13px; font-weight: 800; color: #1a73e8; text-transform: uppercase; letter-spacing: 0.06em;">About IME365</span><h1 style="font-size: 36px; font-weight: 800; color: #0f172a; margin: 8px 0 12px 0;">Empowering Career Excellence</h1><p style="color: #64748b; font-size: 14px; margin: 0;">Built for modern job seekers, engineers, leaders, and career changers.</p></div><section style="margin-bottom: 32px;"><h2 style="font-size: 20px; font-weight: 700; color: #0f172a; margin-bottom: 12px;">Our Mission</h2><p>IME365 combines state-of-the-art LLM intelligence, recruiter-certified typography, and real-time ATS scoring to ensure your talent gets recognized by top employers worldwide.</p></section></div>`,
        published: true,
        status: 'published'
    },
    'about': {
