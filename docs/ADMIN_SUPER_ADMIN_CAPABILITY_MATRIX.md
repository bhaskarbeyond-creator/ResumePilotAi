...
- `/api/platform` is the global Admin/Super Admin operational API. `/api/admin` is the legacy/consumer moderation API. `/api/enterprise` is tenant-aware and feature-gated. Browser aliases `/admin/*` and `/platform/*` redirect to canonical `/adm/*` without changing authorization.
