...
**Scope:** `/adm`, `/admin` alias, Admin settings, platform control plane, user/tenant administration.
...
- Added a consistent write-only secret pattern: configured badge, masked status, blank-preserves explanation, explicit clear, success/error status, and no raw response.
