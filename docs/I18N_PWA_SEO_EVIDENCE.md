...
- `node scripts/audit-ui-strings.mjs` now produces a line-addressable inventory at `docs/I18N_UI_STRING_INVENTORY.json`. The deterministic pass scanned 280 JS/JSX component files and classified 2,896 literal occurrences: 2,369 English UI candidates, 302 accessibility labels, 199 technical/identifier values, and 26 recognized brand/provider names. Dynamic expressions and multiline content remain explicitly marked as requiring source review.
- Existing English fallback is preserved rather than inventing translations or changing legal/payment meaning. The 2,671 UI/accessibility candidates require product-approved source copy and professional translation review, especially billing, deletion, privacy, MFA, Admin, and validation language.
