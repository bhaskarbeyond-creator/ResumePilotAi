...
if (isset($_SERVER['HTTP_X_NVIDIA_API_KEY'])) {
    $customApiKey = $_SERVER['HTTP_X_NVIDIA_API_KEY'];
}
...
if (function_exists('getallheaders')) {
    ...
    foreach ($allHeaders as $name => $value) {
        ...
        if ($lowerName === 'authorization') {
            $authHeader = $value;
        } elseif ($lowerName === 'x-nvidia-api-key') {
            $customApiKey = $value;
        }
        if (in_array($lowerName, ['content-type', 'accept'])) {
            $headers[] = "$name: $value";
        }
    }
}

if ($customApiKey && !$authHeader) {
    $authHeader = "Bearer " . trim($customApiKey);
}
