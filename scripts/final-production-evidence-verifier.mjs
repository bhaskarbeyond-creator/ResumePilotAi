...
async function runMasterVerification() {
    ...
    // ─────────────────────────────────────────────────────────────────
    // 4. REAL SYNCHRONIZATION (MySQL -> Outbox -> Firestore)
    // ─────────────────────────────────────────────────────────────────
    ...
    const hash = calculateContentHash('resumes', { id: syncTestResId, title: syncTestTitle });
    ...
    await pool.query(\`
        INSERT INTO sync_outbox (id, entity_type, entity_id, operation, payload, version, source_engine, content_hash, status, retry_count, max_retries)
        VALUES (?, 'unknown_invalid_type', 'fake_id', 'UPSERT', '{}', 1, 'mysql', 'fake_hash', 'PENDING', 4, 5)
    \`, [invalidEventId]);
    ...
    await pool.query(\`
        INSERT INTO sync_conflicts (id, entity_type, entity_id, mysql_version, firestore_version, mysql_hash, firestore_hash, resolution)
        VALUES (?, 'resumes', 'res_conflict_001', 5, 6, 'hash_mysql_a', 'hash_firestore_b', 'PENDING')
    \`, [conflictId]);
    ...
    console.log('Durable Engine State in MariaDB:', stateRow[0]);
...
fs.writeFileSync('scripts/remote_master_verifier.js', remoteCode);
execSync('scp -o BatchMode=yes scripts/remote_master_verifier.js airesume:~/backend/remote_master_verifier.js');
const output = execSync('ssh -o BatchMode=yes airesume "cd ~/backend && /opt/alt/alt-nodejs20/root/usr/bin/node remote_master_verifier.js"').toString();
console.log(output);
execSync('ssh -o BatchMode=yes airesume "rm -f ~/backend/remote_master_verifier.js"');
fs.unlinkSync('scripts/remote_master_verifier.js');
