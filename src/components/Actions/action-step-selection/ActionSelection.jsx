...
// Preview artwork, keyed by template id. The catalog (derived from the render
// engine's own theme presets) supplies every other card attribute so a card can
// never again describe a different design from the one that renders.
...
const RESUME_TEMPLATE_CARDS = TEMPLATE_CATALOG.map((entry) => ({
    id: entry.id,
    src: RESUME_PREVIEWS[entry.id],
    category: entry.category,
    name: entry.name,
    description: entry.description,
    popularity: entry.popularity,
    accessibleLabel: templateAccessibleLabel(entry.id),
}));
                ...
                <div className="p-3">
                    <div className="flex items-center justify-between mb-1">
                        <div className="flex flex-col min-w-0 flex-1">
                            <h3 className="font-semibold text-gray-900 text-sm truncate">{template.name || template.id}</h3>
                            <span className="text-[11px] font-bold text-[#694aff] tracking-wider uppercase mt-0.5">
                                (CV #{template.id.replace(/^Cv/i, '')})
                            </span>
                        </div>
                        {isSelected && (
