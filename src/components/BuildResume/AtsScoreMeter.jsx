...
const RadialGauge = ({ score = 0, stroke = '#475569', label = '' }) => {
    const radius = 22;
    const circumference = 2 * Math.PI * radius;
    const safeScore = typeof score === 'number' && Number.isFinite(score) ? score : 0;
    const offset = circumference - (Math.max(0, Math.min(100, safeScore)) / 100) * circumference;
    return (
        <div className="relative flex h-12 w-12 flex-shrink-0 items-center justify-center">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-xs font-black leading-none text-slate-900">{safeScore}</span>
...
const AtsScoreMeter = ({ resumeData = {}, onNavigate }) => {
    ...
    const result = useMemo(
        () => calculateAtsScore(resumeData, { jobDescription: jdText }) || {
            ...
            jdMatch: { score: null, matched: [], missing: [], groups: {} },
            language: { nonEnglish: false },
            improvements: [],
            strengths: [],
            sections: [],
        },
    ...
    const gaugeLabel = t(
        ...
        { score: result?.qualityScore || 0, status: result?.status?.label || 'Getting Started' },
    );
        ...
        <section
            ...
            <div className="flex items-center justify-between gap-2">
                <div className="flex min-w-0 items-center gap-2">
                    <RadialGauge score={result?.qualityScore || 0} stroke={theme.stroke} label={gaugeLabel} />
                    <div className="min-w-0">
                        ...
                        <span className={`mt-0.5 inline-block max-w-full truncate rounded-md border px-1.5 py-0.5 text-[10px] font-extrabold ${theme.chip}`}>
                            {result?.status?.label || 'Getting Started'}
                        </span>
                    </div>
                </div>
                <button
            ...
            {isExpanded && (
                <div id="ats-score-details" className="max-h-[min(22rem,50vh)] space-y-2.5 overflow-y-auto border-t border-slate-100 pt-2">
                        ...
                        <div>
                            ...
                            <ol className="space-y-1">
                                {(result.improvements || []).map((item, index) => (
                                    <li key={item.text || index} className="text-[10px] leading-snug text-slate-700">
                    ...
                    {(result?.strengths || []).length > 0 && (result?.qualityScore || 0) >= 15 && (
                        <div>
                            <h3 className="mb-1 text-[10px] font-bold uppercase tracking-wider text-slate-400">
                    ...
                    <div className="space-y-1.5">
                        ...
                        {((result?.qualityScore || 0) < 15 ? (result?.sections || []).filter((section) => ['contact', 'experience', 'summary'].includes(section.id)) : (result?.sections || [])).map((section) => {
                            const percent = Math.round(((section.score || 0) / (section.maxScore || 1)) * 100);
                                    ...
                                    <div className="space-y-1.5 pt-1">
                                        <div className="flex justify-between text-[11px] font-bold">
                                            <span className="text-emerald-700">
                                                {t('AtsScoreMeter.matched', 'Matched')} ({result.jdMatch.matched?.length || 0})
                                            </span>
                                            <span className="text-amber-700">
                                                {t('AtsScoreMeter.missing', 'Missing')} ({result.jdMatch.missing?.length || 0})
                                            </span>
                                        </div>
