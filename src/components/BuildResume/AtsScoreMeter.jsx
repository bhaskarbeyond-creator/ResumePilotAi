...
const AtsScoreMeter = ({ resumeData, onNavigate }) => {
    ...
    const gaugeLabel = t(
        ...
        'ATS score {{score}} out of 100, {{status}}',
        { score: result.totalScore, status: result.status.label },
    );
    ...
    return (
        <section
            className="space-y-3 rounded-2xl border border-slate-200/90 bg-white p-3.5 shadow-sm"
            aria-label={t('AtsScoreMeter.region', 'ATS score')}>
            <div className="flex items-center justify-between gap-2.5">
                <div className="flex min-w-0 items-center space-x-2.5">
                    <RadialGauge score={result.totalScore} stroke={theme.stroke} label={gaugeLabel} />
                    <div className="min-w-0">
                        ...
                        <span className={`mt-0.5 inline-block max-w-full truncate rounded-md border px-2 py-0.5 text-[10px] font-extrabold ${theme.chip}`}>
                            {result.status.label}
                        </span>
                    </div>
                </div>
            ...
            {isExpanded && (
                <div id="ats-score-details" className="max-h-[28rem] space-y-3 overflow-y-auto border-t border-slate-100 pt-2">
                            ...
                            return (
                                <article key={section.id} className="space-y-1 rounded-xl border border-slate-100 bg-slate-50/70 p-2">
                                    <div className="flex items-center justify-between text-xs">
                                        <h4 className="truncate text-[11px] font-bold text-slate-700">{section.name}</h4>
                                    ...
                                    <ul className="space-y-0.5">
                                        {section.findings.slice(0, 3).map((finding) => (
                                            <li key={finding.text} className={`text-[10px] leading-tight ${finding.ok ? 'text-slate-600' : 'text-amber-800'}`}>
                                                {finding.ok ? '✓' : '⚠'} {finding.text}
                                            </li>
                                        ))}
                                    </ul>
                                    <p className="pt-0.5 text-[10px] leading-tight text-slate-500">{section.action}</p>
                                    {section.navigateTo && typeof onNavigate === 'function' && section.score < section.maxScore && (
                                        <button
                                            ...
                                            {t('AtsScoreMeter.improve', 'Improve')} {section.name}
                                        </button>
                                    )}
                                </article>
                            );
                    ...
                    <div className="border-t border-slate-200 pt-2">
                        <button
                            ...
                            <span>{t('AtsScoreMeter.jdTitle', 'Target job description')}</span>
                            <span className="rounded bg-indigo-200/60 px-1.5 py-0.5 text-[10px] text-indigo-900">
                                {result.jdMatch.score != null
                                    ? t('AtsScoreMeter.jdMatchPct', '{{score}}% match', { score: result.jdMatch.score })
                                    : t('AtsScoreMeter.scanJd', 'Optional')}
                            </span>
                        </button>
                        ...
                        {showMatcher && (
                            <div className="mt-2 space-y-2 rounded-xl border border-slate-200 bg-slate-50 p-2.5">
                                ...
                                <textarea
                                    ...
                                    className="h-20 w-full resize-none rounded-lg border border-slate-200 bg-white p-2 text-xs focus:outline-none focus:ring-1 focus:ring-indigo-500"
                                />
                                    ...
                                    <div className="space-y-1.5 pt-1">
                                        ...
                                        {result.jdMatch.matched.length > 0 && (
                                            <div className="flex flex-wrap gap-1">
                                                {result.jdMatch.matched.slice(0, 12).map((keyword) => (
                                        ...
                                        {Object.keys(result.jdMatch.groups || {}).map((group) => (
                                            <div key={group}>
                                                <p className="mb-0.5 text-[9px] font-bold uppercase tracking-wide text-slate-500">{group}</p>
                                                <div className="flex flex-wrap gap-1">
                                                    {result.jdMatch.groups[group].slice(0, 8).map((keyword) => (
