...
import { getRoleHealth } from '../../../../utils/bulletQuality.js';
import { MdCheckCircle, MdWarningAmber, MdTrendingUp, MdAutoAwesome } from 'react-icons/md';

/**
 * RoleHealthCard
 * Displays instant, role-level ATS readiness feedback for a single work experience entry.
 * Evaluates Title, Employer, Chronology, Bullet Count, Action Verbs, Metrics, and JD alignment.
 */
export default function RoleHealthCard({ employment = {}, targetJd = '' }) {
    const health = useMemo(() => {
        return getRoleHealth(employment, targetJd);
    }, [employment, targetJd]);

    if (!employment.jobTitle && !employment.employer && health.bulletCount === 0) {
        return null;
    }
        ...
        <div className="rounded-xl border border-slate-200/80 bg-slate-50/70 p-3 space-y-2 transition-all">
            ...
            <div className="flex items-center justify-between gap-2 flex-wrap">
                <div className="flex items-center gap-2">
                    <span className="text-[11px] font-bold text-slate-700 uppercase tracking-wider flex items-center gap-1.5">
                        <MdTrendingUp className="w-3.5 h-3.5 text-indigo-600" />
                        Role ATS Health
                    </span>
            ...
            <div className="flex flex-wrap gap-1.5 pt-0.5">
                {health.pills.map((pill) => {
                    ...
                    } else if (pill.color === 'red') {
                        style = 'bg-red-50/90 text-red-800 border-red-200';
                    }
