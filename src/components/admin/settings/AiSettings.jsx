                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 border border-slate-200 animate-in fade-in zoom-in duration-200">
                        <div className="flex items-center space-x-3 mb-4">
                            ...
                            <div>
                                <h3 className="text-base font-bold text-slate-800">Security Verification Required</h3>
                                <p className="text-xs text-slate-500">Re-authenticate to complete this sensitive admin action</p>
                            </div>
                        </div>

                        <p className="text-xs text-slate-600 mb-4 bg-slate-50 p-3 rounded-lg border border-slate-200">
                            Your admin session token requires fresh authentication before changing or testing provider credentials.
                        </p>

                        <form onSubmit={(e) => { e.preventDefault(); handleReauthenticateAndRetry(); }} className="space-y-4">
                            {usesPasswordProvider && (
                                <div>
                                    <label className="block text-xs font-semibold text-slate-700 mb-1">Enter Password</label>
                                    <input
                                        type="password"
                            ...
                            <div className="flex items-center justify-end space-x-2 pt-2">
                                <button
                                    type="button"
                                    onClick={() => { setPendingOperation(null); setReauthPassword(''); }}
                                ...
                                <button
                                    type="submit"
                                    disabled={reauthenticating || (usesPasswordProvider && !reauthPassword.trim())}
                                    className="px-5 py-2 text-xs font-semibold text-white bg-slate-900 hover:bg-black rounded-lg flex items-center space-x-2 shadow-md disabled:opacity-50"
                                >
                                    {reauthenticating && <FaSpinner className="animate-spin text-white mr-1" />}
