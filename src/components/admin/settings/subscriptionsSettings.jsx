...
import { getSubscriptionStatus, setSubscriptionsData, getAllCouponsAdmin, saveCoupon, deleteCoupon, getSystemSettings, saveSystemSettings, getAllInvoicesAdmin } from '../../../firestore/dbOperations';
import { FaCheck, FaTimes, FaCreditCard, FaRupeeSign, FaDollarSign, FaToggleOn, FaToggleOff, FaPaypal, FaStripe, FaFlask, FaShieldAlt, FaTag, FaPlus, FaTrash, FaEdit, FaCalendarAlt, FaPercent, FaEye, FaEyeSlash, FaDownload, FaSearch, FaFileInvoice, FaPrint, FaListAlt, FaCog } from 'react-icons/fa';
...
class SubscriptionSetting extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            adminTab: 'settings', // 'settings' or 'invoices'
            adminInvoicesList: [],
            invoiceSearchQuery: '',
            ...
            <div className="space-y-6 max-w-5xl mx-auto p-4 sm:p-6">
                ...
                <div className="flex items-center justify-between border-b border-slate-200 pb-3 gap-2">
                    <div className="flex items-center space-x-2">
                        <button
                            type="button"
                            onClick={() => this.setState({ adminTab: 'settings' })}
                            className={`flex items-center gap-2 px-4 py-2.5 rounded-xl font-bold text-xs transition-all cursor-pointer ${
                                this.state.adminTab === 'settings'
                                    ? 'bg-indigo-600 text-white shadow-md'
                                    : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
                            }`}
                        >
                            <FaCog className="w-4 h-4" />
                            <span>Payment &amp; GST Settings</span>
                        </button>
                        <button
                            type="button"
                            onClick={() => {
                ...
                {/* --- TAB 2: INVOICES AUDIT LEDGER --- */}
                {this.state.adminTab === 'invoices' && (
                    <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-2xs space-y-5">
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-4">
                            <div>
                                <h3 className="text-lg font-extrabold text-slate-900 flex items-center gap-2">
                                    <FaFileInvoice className="w-5 h-5 text-indigo-600" />
                                    Master Invoices &amp; Transactions Audit Ledger
                                </h3>
                                <p className="text-xs text-slate-500 mt-0.5">Live site-wide customer GST invoices, B2B tax details, and gateway payment records</p>
                            </div>
                            <div className="flex items-center gap-2">
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        placeholder="Search invoice no, candidate, GSTIN..."
                                        className="pl-9 pr-3 py-2 text-xs bg-slate-50 border border-slate-200 rounded-xl w-64 focus:bg-white focus:border-indigo-500 outline-none"
                                    />
                                </div>
                                <select
                        ...
                        {this.state.loadingInvoices ? (
                            ...
                            const filter = this.state.invoiceTypeFilter;
                            const filtered = (this.state.adminInvoicesList || []).filter(inv => {
                                const invNo = (inv.invoiceNumber || inv.invoiceId || inv.id || '').toLowerCase();
                                ...
                                const matchesFilter = filter === 'all' || (filter === 'b2b' && isB2B) || (filter === 'b2c' && !isB2B);
                            ...
                            if (filtered.length === 0) {
                                return (
                                    <div className="p-12 text-center border-2 border-dashed border-slate-200 rounded-xl bg-slate-50/50">
                                        <FaFileInvoice className="w-10 h-10 text-slate-300 mx-auto mb-2" />
                                        <p className="text-sm font-bold text-slate-700">No Invoices Found</p>
                                        <p className="text-xs text-slate-500 mt-1">Customer invoices will populate automatically when users complete checkouts.</p>
                                    </div>
                                );
                            }
                            ...
                            return (
                                <div className="overflow-x-auto">
                                    <table className="w-full text-left border-collapse">
                                        <thead>
                                            <tr className="bg-slate-100/70 border-b border-slate-200 text-[11px] font-black text-slate-600 uppercase tracking-wider">
                                                <th className="p-3">Invoice No</th>
                                                <th className="p-3">Date</th>
                                                <th className="p-3">Customer</th>
                                                <th className="p-3">Type &amp; GSTIN</th>
                                        ...
                                        <tbody className="divide-y divide-slate-100 text-xs">
                                            {filtered.map(inv => {
                                                ...
                                                const invNo = inv.invoiceNumber || inv.invoiceId || inv.id;
                                                const taxable = inv.taxableAmount !== undefined ? inv.taxableAmount : (inv.subtotal || inv.amount);
                                                const tax = inv.totalTax !== undefined ? inv.totalTax : (inv.taxAmount || 0);
                                                const total = inv.grandTotal !== undefined ? inv.grandTotal : (inv.amount || 0);

                                                return (
                                                    <tr key={inv.id} className="hover:bg-slate-50/80 transition-colors">
                                                        <td className="p-3 font-mono font-bold text-indigo-950">{invNo}</td>
                                                        <td className="p-3 text-slate-600">{inv.formattedDate || inv.date || 'Recent'}</td>
                                                        <td className="p-3">
                                                            <div className="font-bold text-slate-900">{inv.customerSnapshot?.name || inv.customerName || 'Valued Customer'}</div>
                                                            <div className="text-[10px] text-slate-500">{inv.customerSnapshot?.email || inv.customerEmail}</div>
                                                        </td>
                                                        <td className="p-3">
                                                            <span className={`px-2 py-0.5 rounded text-[10px] font-extrabold uppercase ${isB2B ? 'bg-purple-100 text-purple-800' : 'bg-slate-100 text-slate-700'}`}>
                                                                {isB2B ? 'B2B' : 'B2C'}
                                                            </span>
                                                            {isB2B && <div className="font-mono text-[10px] text-slate-600 mt-0.5">{gstin}</div>}
                                                        </td>
                                                        <td className="p-3 font-mono text-slate-700">{symbol}{parseFloat(taxable).toFixed(2)}</td>
                                                        <td className="p-3 font-mono text-slate-700">{symbol}{parseFloat(tax).toFixed(2)}</td>
                                                        <td className="p-3 font-mono font-extrabold text-slate-900">{symbol}{parseFloat(total).toFixed(2)}</td>
                                                        <td className="p-3">
                                                        ...
                                                        <td className="p-3 text-right">
                                                            <button
                                                                ...
                                                                onClick={() => {
                                                                    ...
                                                                    if (win) {
                                                                        win.document.write(`<html><head><title>Invoice ${invNo}</title></head><body><h2 style="font-family:sans-serif;">Tax Invoice ${invNo}</h2><p style="font-family:sans-serif;">Customer: ${inv.customerSnapshot?.name || inv.customerName || 'Customer'}</p><p style="font-family:sans-serif;">Amount: ${symbol}${total}</p><script>window.print();</script></body></html>`);
                                                                        win.document.close();
                                                                    }
                                                                }}
