...
export default function UsersManager() {
  ...
  const [provisionForm, setProvisionForm] = useState({ displayName: '', email: '', temporaryPassword: '', confirmTemporaryPassword: '' });
  ...
  const provisionUser = async event => {
    event.preventDefault();
    if (provisionForm.temporaryPassword !== provisionForm.confirmTemporaryPassword) {
      setMessage({ type: 'error', text: 'The temporary password confirmation does not match.' });
      return;
    }
    setRunning(true);
    setMessage(null);
    try {
      const result = await provisionUserBySuperAdmin(provisionForm);
      if (!result.success) throw new Error(result.error || 'User provisioning failed.');
      setProvisionOpen(false);
      setProvisionForm({ displayName: '', email: '', temporaryPassword: '', confirmTemporaryPassword: '' });
      setMessage({ type: 'success', text: `Provisioned ${result.user?.email || 'the user'} as a standard USER. Share the temporary password only through an approved secure channel.` });
      await loadUsers(1);
    } catch (error) {
      setMessage({ type: 'error', text: error.message || 'User provisioning failed.' });
    } finally {
      setRunning(false);
    }
  };
  ...
  return (
    <main className="mx-auto max-w-7xl space-y-6">
      <header className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-2xs sm:flex-row sm:items-start sm:justify-between sm:p-6">
        ...
        <div className="flex flex-wrap gap-2">{canManageRoles && <button type="button" onClick={() => { setProvisionForm({ displayName: '', email: '', temporaryPassword: '', confirmTemporaryPassword: '' }); setProvisionOpen(true); }} disabled={running} className="rounded-xl bg-indigo-600 px-3.5 py-2 text-xs font-bold text-white hover:bg-indigo-700 disabled:opacity-50">Provision standard user</button>}<button type="button" onClick={() => loadUsers(pagination.page)} disabled={loading || running} className="rounded-xl border border-slate-300 bg-white px-3.5 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50 disabled:opacity-50">{loading ? <FaSpinner className="inline animate-spin" /> : <FaUsers className="inline" />} Refresh</button><button type="button" onClick={exportUsersToCsv} disabled={!users.length} className="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-3.5 py-2 text-xs font-bold text-white hover:bg-slate-800 disabled:opacity-50"><FaDownload />Export current page</button></div>
      </header>
      ...
      <AdminDialog open={provisionOpen} onClose={() => !running && setProvisionOpen(false)} dismissible={!running} title="Provision standard user" description="Creates a Firebase email/password USER identity. It cannot create an ADMIN or SUPER_ADMIN identity, and recent authentication is required." className="max-w-lg"><form onSubmit={provisionUser} className="space-y-4 p-5"><label className="block text-sm font-bold text-slate-800">Display name<input required maxLength="100" value={provisionForm.displayName} onChange={event => setProvisionForm(current => ({ ...current, displayName: event.target.value }))} className="mt-2 w-full rounded-xl border border-slate-300 px-3 py-2 font-normal" /></label><label className="block text-sm font-bold text-slate-800">Email<input required type="email" autoComplete="off" value={provisionForm.email} onChange={event => setProvisionForm(current => ({ ...current, email: event.target.value }))} className="mt-2 w-full rounded-xl border border-slate-300 px-3 py-2 font-normal" /></label><label className="block text-sm font-bold text-slate-800">Temporary password<input required type="password" minLength="12" maxLength="128" autoComplete="new-password" value={provisionForm.temporaryPassword} onChange={event => setProvisionForm(current => ({ ...current, temporaryPassword: event.target.value }))} className="mt-2 w-full rounded-xl border border-slate-300 px-3 py-2 font-normal" /><span className="mt-1 block text-[11px] font-normal text-slate-500">12–128 characters; do not include the email name. The password is never returned, stored in UI state after success, or written to audit metadata.</span></label><label className="block text-sm font-bold text-slate-800">Confirm temporary password<input required type="password" minLength="12" maxLength="128" autoComplete="new-password" value={provisionForm.confirmTemporaryPassword} onChange={event => setProvisionForm(current => ({ ...current, confirmTemporaryPassword: event.target.value }))} className="mt-2 w-full rounded-xl border border-slate-300 px-3 py-2 font-normal" /></label><div className="flex justify-end gap-3 border-t border-slate-100 pt-4"><button type="button" disabled={running} onClick={() => setProvisionOpen(false)} className="rounded-xl border border-slate-300 px-4 py-2 text-sm font-bold text-slate-700">Cancel</button><button type="submit" disabled={running} className="inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-2 text-sm font-bold text-white disabled:opacity-50">{running && <FaSpinner className="animate-spin" />}{running ? 'Provisioning…' : 'Create user'}</button></div></form></AdminDialog>
