        ...
        for (let index = 0; index < (this.props.values.skills || []).length; index++) {
            ...
            elements.push(
                <div key={index} className="cv6-skillItem">
                    ...
                    {rating !== null && (
                        <div className="cv6-ratingWrapper" style={{ border: `1px solid ${this.getPrimaryColor()}` }} aria-label={`${rating}% proficiency`}>
                            <div style={{ width: `${rating}%`, backgroundColor: this.getPrimaryColor() }} className="cv6-rating"></div>
                        </div>
                    )}
                </div>
            );
