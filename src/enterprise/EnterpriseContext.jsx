...
export function EnterpriseTenantProvider({ children }) {
  ...
  const [state, setState] = useState({ loading: enabled, error: null, tenants: [], workspaces: [], context: null, tenant: null, workspace: null });

  const load = useCallback(async ({ tenantId = '', workspaceId = '' } = {}) => {
    if (!enabled || !user?.uid) {
      setState({ loading: false, error: null, tenants: [], workspaces: [], context: null, tenant: null, workspace: null });
      return null;
    }
    ...
    try {
      ...
      setState(previous => ({ ...previous, loading: false, error, workspaces: [], context: null, tenant: null, workspace: null }));
      throw error;
    }
  }, [enabled, user?.uid]);
