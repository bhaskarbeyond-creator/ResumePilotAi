...
export async function getUserById(identifier) {
    ...
    try {
        ...
        const isAdmin = ['ADMIN', 'SUPER_ADMIN'].includes(String(claims?.claims?.role || '').toUpperCase()) || claims?.claims?.permissions?.includes('*');
        if (isAdmin) {
    ...
    } catch (_) { /* fall through to the owner-scoped lookup */ }
    const db = fire.firestore();
...
function redactClientSecrets(settings = {}) {
    ...
    const publicKeyFields = new Set([
        'apiKey', 'googleMapsApiKey', 'cloudinaryApiKey', 'stripePublishableKey',
    ...
    const secretField = key => /(?:secret|password|privateKey|authToken|clientToken|accessToken|refreshToken|serviceAccount|merchantKey|saltKey|keySecret|webhookSecret|s3AccessKey|apiKey)$/i.test(key)
        && !publicKeyFields.has(key);
    const redact = value => {
        if (Array.isArray(value)) return value.map(redact);
        if (!value || typeof value !== 'object') return value;
        return Object.fromEntries(Object.entries(value).filter(([key]) => !secretField(key)).map(([key, item]) => [key, redact(item)]));
    };
    return redact(copy);
}
