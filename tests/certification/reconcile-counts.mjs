...
function parseNodeTest(stdout) {
  const get = (key) => {
    const m = stdout.match(new RegExp(`^(?:#|ℹ)\\s+${key}\\s+(\\d+)`, 'm'));
...
function runNodeTest(files, opts = {}) {
  const r = spawnSync(process.execPath, ['--test', '--test-force-exit', '--test-concurrency=1', ...files], {
    cwd: opts.cwd || ROOT, encoding: 'utf8', env: { ...process.env, ...LD, ...(opts.env || {}) }, timeout: opts.timeout || 900_000,
  });
...
results['enterprise'] = runNodeTest(listTestFiles(path.join(ROOT, 'backend', 'enterprise-test')), { cwd: path.join(ROOT, 'backend') });

console.log('== frontend-security ==');
const secFiles = ['tests/security-static.test.mjs', 'tests/secret-scanner-efficacy.test.mjs', 'tests/xss.test.mjs', 'tests/mfa-static.test.mjs'];
let secTotal = { tests: 0, pass: 0, fail: 0, cancelled: 0, skipped: 0, todo: 0 };
let secExit = 0;
for (const f of secFiles) {
  const res = runNodeTest([f], { timeout: 120_000 });
  secTotal.tests += res.counts.tests;
  secTotal.pass += res.counts.pass;
  secTotal.fail += res.counts.fail;
  if (res.exitCode !== 0) secExit = res.exitCode;
}
results['frontend-security'] = { counts: secTotal, exitCode: secExit };
...
for (const f of runtimeFiles) {
  ...
  rcTotal.fail += res.counts.fail;
  if (res.exitCode !== 0) rcExit = res.exitCode;
}
results['runtime-cert'] = { counts: rcTotal, exitCode: rcExit };
...
console.log('== frontend-product ==');
{
  ...
  const r = spawnSync(npmCmd, ['run', 'test:product'], { cwd: ROOT, encoding: 'utf8', env: { ...process.env, ...LD }, timeout: 900_000, shell: true });
  const out = r.stdout || '';
  // Sum across all node:test invocations in the script.
  let tests = 0, pass = 0, fail = 0;
  for (const m of out.matchAll(/^(?:#|ℹ)\s+tests\s+(\d+)/gm)) tests += Number(m[1]);
  for (const m of out.matchAll(/^(?:#|ℹ)\s+pass\s+(\d+)/gm)) pass += Number(m[1]);
  for (const m of out.matchAll(/^(?:#|ℹ)\s+fail\s+(\d+)/gm)) fail += Number(m[1]);
