...
test('job application lifecycle is backend-owned, identity-bound, and atomic', async () => {
  const [backend, employer, operations, policy, migration] = await Promise.all([
  ...
  const submit = employer.match(/router\.post\('\/jobs\/:jobId\/applications'[\s\S]*?\n\s*\}\);/)?.[0] || '';
  ...
  const status = employer.match(/router\.patch\('\/job-applications\/:applicationId\/status'[\s\S]*?\n\s*\}\);/)?.[0] || '';
...
test('employer job posting mutations are backend-owned, audited, and revision safe', async () => {
  const [backend, employer, operations, policy, migration, dashboard, editor, companies] = await Promise.all([
    fs.readFile('backend/index.js', 'utf8'),
    fs.readFile('backend/routes/employer.js', 'utf8'),
    fs.readFile('src/services/api/platform.js', 'utf8'),
    fs.readFile('backend/security/policy.js', 'utf8'),
  ...
  const allBackend = backend + '\n' + employer;
  assert.match(allBackend, /EMPLOYER_COMPANY_CREATED/);
  assert.match(allBackend, /EMPLOYER_COMPANY_EDITED/);
  assert.match(allBackend, /EMPLOYER_COMPANY_DELETED/);
  assert.match(allBackend, /COMPANY_HAS_JOBS/);
  assert.match(allBackend, /EMPLOYER_JOB_CREATED/);
  assert.match(allBackend, /EMPLOYER_JOB_STATUS_CHANGED/);
  assert.match(allBackend, /EMPLOYER_JOB_EDITED/);
  assert.match(allBackend, /EMPLOYER_JOB_DELETED/);
  assert.match(allBackend, /EMPLOYER_JOB_CHANGED/);
  ...
  assert.match(employer, /isEmployerAccount\(req\)/);
  assert.match(employer, /employerId: req\.user\.uid/);
