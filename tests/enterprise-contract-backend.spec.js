// @ts-check
/**
 * AUTHENTICATED Playwright contract suite against the real Express stack.
 *
 ...
 * application (backend/index.js) running with the enterprise middleware
 * chain, control plane, RBAC, M2M authentication and explicit MariaDB-contract test doubles. Authentication is real end-to-end: a seeded Firebase-style session
 * produces a bearer token that the backend's middleware verifies.
 *
 * Evidence captured:
 *  - all 14 enterprise modules render from Express contract backend data
 *  - resume CRUD through the UI persists in the MariaDB-compatible test contract
...
const { startEnterpriseContractBackend } = require('./helpers/enterprise-contract-backend.cjs');

test.describe.configure({ mode: 'serial' });

const SHOTS = path.join('scratch', 'contract-backend-audit');
const MODULES = ['overview', 'resumes', 'members', 'teams', 'workspaces', 'access', 'ai', 'security', 'usage', 'email', 'audit', 'support', 'settings', 'platform'];

/** @type {ReturnType<typeof startEnterpriseContractBackend> extends Promise<infer T> ? T : never} */
let backend;
let vite;
let base = '';
...
test.beforeAll(async () => {
  ...
  backend = await startEnterpriseContractBackend();
...
test('console boots with a real tenant resolved by the Express contract backend', async ({ page }) => {
...
// ---------------------------------------------------------------------------
// 2. All modules render from Express contract backend data
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// 3. Resume CRUD against the MariaDB-compatible test contract
// ---------------------------------------------------------------------------
