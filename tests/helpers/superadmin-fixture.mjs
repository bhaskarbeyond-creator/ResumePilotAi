...
export function seedOperationalSnapshot(now = new Date().toISOString()) {
  const services = [
    ...
    {
      id: 'firestore', name: 'Firestore', group: 'core', state: 'OPERATIONAL', support: 'SUPPORTED', enabled: true,
      configuration: 'CONFIGURED', critical: true, reason: 'Write probe succeeded in 24ms.', dependency: 'Google Cloud Firestore',
    ...
    },
    {
      ...
      remediation: 'Re-run the health check; if this persists, inspect the outbox collection.',
  ...
  return {
    ...
    sources: { firestore: 'ok', email: 'ok', outbox: 'unavailable' },
    services,
    apiMatrix: {
