...
test('conversation lists and messages are selected through caller membership', async () => {
  ...
  const list = routeBlock(source, "app.get('/api/messages/conversations'");
  ...
  const messages = routeBlock(source, "app.get('/api/messages/conversations/:conversationId/messages'");
...
test('message writes reject non-participants and derive sender identity from the verified token', async () => {
  const source = await fs.readFile('backend/index.js', 'utf8');
  const send = routeBlock(source, "app.post('/api/messages/send'");
...
test('conversation creation is limited to the application parties and idempotent', async () => {
  const source = await fs.readFile('backend/index.js', 'utf8');
  const create = routeBlock(source, "app.post('/api/messages/conversations'");
